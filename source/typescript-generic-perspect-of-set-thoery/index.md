...
我按照文中的目标和步骤进行一些实践和思考，然后似乎领悟到一些关于 TS 泛型的**更接近实质**的知识 —— 从集合的视角来看泛型。基于这些知识，我发现原文中的大部分泛型都有更严密的写法。我觉得这次实践和思考的过程值得记录下来，因此有了本文。

和原文一样，本文并不讨论柯里化和函数式编程，柯里化只是用来讨论 TS 泛型编程的素材。本文将逐条解释我自己的这份完整实现中的每一个泛型的意义，但这不是重点，重点是背后的领悟 —— 在文章前半部分，我常常会围绕一条简单的泛型讨论较长篇幅，请不要直接跳过。

### 命题

柯里化是函数式编程领域的一个概念，它表示这样的过程：把一个多参数的函数转化成「一次接收一个参数」的函数，比如把 `f(a,b,c)` 转化为 `f(a)(b)(c)`。举个更详细的例子：
...
柯里化转换函数 `curry` 都是柯里化的核心。转换函数接收一个普通函数 `toCurry` —— 暂时用 `Function` 类型来表示，返回柯里化转换后的函数 `curried`（后面也称**柯里化的函数**或**柯里化函数**）。柯里化的函数暂时用 `unknown` 类型来表示，如何写出它的合法的类型表达，就是这篇文章的主线。
...
集合的映射，必须基于作用于**集合内元素**的**规则**。假设有集合 A 和 B，只有当 A 中的任意元素，按照**某种规则**，在 B 中恒有唯一元素与之对应，我们才能说 A 和 B 之间存在映射关系。

> 既然映射只能从**一个**集合映射到另一个集合，如何理解多个泛型参数的情况？答案：把多个泛型参数看作一个元组类型。
...
将类型 `unknown[]` 传递给 `Length` 得到类型 `number`，这是因为：属于类型 `unknown[]` 的无数个元素中的任意一个，不管是 `[]` 还是 `[1]` 还是 `['foo', true]`，**对它求取长度**，得到的结果 `0`，`1` 或 `2` 等等，都是 `number` 类型。注意，`Length` 不能保证经过规则转换后的值**占满**映射得到的集合：没有什么数组的长度是 `0.5` 或 `-1`。所以，泛型运算 `Length<unknown[]>` 的结果 `number` 其实是真实世界中映射得到的结果集的**超集**。

泛型返回「真实结果集」的超集，时常比我们预期的类型要宽泛，这是不可避免的。从集合的角度看，编写泛型的目的，就是提供「真实结果集」的**可以用类型规则描述**的**尽可能小**的超集。理解这一点很重要。

> 我曾想，如果 JS 支持无符号整数类型，`Length<unknown[]>` 就可以得到完美的结果集，但这其实只是巧合。绝大多数场合是无法描述完美结果集的：比如 `Length<[unknown, ...unknown[]]>` 需要返回由大于 1 的整数构成的集合。
...
我曾疑问：为什么条件泛型里只有 `extends` 关键字，而没有 `equals` 关键字或 `==` 运算符？现在我的领悟是：在集合运算中，只有**包含运算**才能产生「是」或「否」的结果，而集合的其他运算：交集、并集、补集、映射，他们的运算结果都是另一个集合。我们说 A 包含 B，即 A 是 B 的子集，切换到类型语境下，就是 A 是 B 的子类，也就是 A 继承自 B。
...
前三条测试的结果是符合直觉的。第四条，当我们把**包含空数组的单元素集**传递给 `Head`，得到的结果是 `never` 类型，表示空集，也没问题。

让我们再看一眼第二条测试：请问空数组是不是 `string[]` 集合的元素？当然是了。那么，在映射 `Head` 中，空数组被映射为了什么元素？

集合论中映射的前提是，映射规则对源集合内的所有元素都生效，映射不能把非空集映射为空集。这应该如何解释？

我的领悟是：TS 中存在一个**写不出来**（JS 中没有）的 **`never` 对象**，而**能写出来**的 **`never` 类型**表示包含 `never` 对象的单元素集。同时，TS 中任何能写出来的类型都隐式包含了 `never` 对象，这使得任何类型与 `never` 类型求并集得到的都是自身，从而使本来是单元素集的 `never` 类型在**概念**上变成了空集。

从这个角度理解 `Head<string[]>` 就说得通了：`string[]` 集合中的空数组元素被映射为了 `never`，而其他元素被映射为了 `string`；因为 `string | never` 依然是 `string`，所以最终返回 `string`。

### 泛型 `Tail`

第三个泛型 `Tail` 取出元组或数组 `T` 的尾部元素（除去第一个元素后剩余的那些元素）的类型。
...
考虑用 `string[]` 测试一下，我们得到了 `never`。这不太对劲。

```typescript
type SimpleTail_Test3 = SimpleTail<string[]>;              // => never
```

根据 `SimpleTail` 的实现：`string[]` 确实不是 `[unknown, ...unknown[]]` 的子集，我们只能返回 `never`。但是，在真实世界中，`string[]` 集合的几乎所有元素（除空数组对象外），取尾项操作都是有意义的，在这里返回空集是不合理的。事实上，如果我们举一些例子进行归纳的话，一定可以得出结论：`Tail<string[]>` 的结果**就**是 `string[]`。
...
1. 如果 `T` 是空数组单元素集的子集，我们可以断定：`T` 只能是空数组单元素集或 `never`，此时返回 `never`；
2. 如果 `T` 是「由所有「拥有第一项的数组」组成的集合」的子集，我们可以断定：`T` 不可能包含空数组元素，此时用类似 `SimpleTail` 中的形式提取出尾项类型。
3. 如果上述两者都不满足，我们直接返回 `T`。

传入 `string[]` 测试一下，类型运算经过了上面的第 3 条规则（两者都不满足），返回了预期的类型：`string[]`。

```typescript
type Tail_Test4 = Tail<string[]>;                           // => string[]
```

你真的确定吗？如果 `T` 即不满足规则 1 也不满足规则 2，就一定是类似于 `string[]` 或 `number[]` 这种纯粹的数组类型，而不可能是**其他**类型吗？

让我们归纳一下数组（包括元组）类型的**写法**：（这里，我们不关心数组元素的具体类型，全部用 `unknown` 代替）
...
我们发现，定义数组类型的写法**只有**上面这 4 种，**没有**其他的了！能够**写出来**的数组类型，能够**算出来**（其他泛型返回）的数组类型，最后都能归纳到其中。这 4 种写法就是 TS 处理数组类型的**边界**，换言之 TS 无法产生「无法用这 4 种写法组合出来」的数组类型。

根据 `Tail` 的实现：只有第 2 种写法（即纯粹的数组类型）能走到最后一个分支。正是基于对以上知识的理解，我们才能放心地在最后一个分支里返回 `T`。
...
根据集合论，并集的映射，应由组成并集的**每个**单个集合，分别映射之后，再对**诸结果集**取并集，作为最终的结果。

`Tail` 没有令我们失望，它正确地返回了预期的类型。但这是有条件的，泛型中的条件需要满足**分发条件类型**的约束：条件必须是泛型参数**直接** `extends` 某个类型（形如 `T extends SOMETYPE`），如果我们把 `Tail` 实现中的第一个条件 `T extends []` 换成 `Length<T> extends 0`，大厦就会坍塌 —— 分发条件类型的约束失效，命题「`T` 只可能是这 4 种写法之一」也不复存在，泛型实现的逻辑就走上了一条失控的道路。
...
你是否已经体会到泛型编程的某种**笨拙**之处？集合映射的规则（即泛型的语义）是基于**集合内元素**的，但泛型的实现者必须基于**集合本身**的运算来回答「映射之后是什么集合」的问题。这需要从真实世界的角度**切实地**思考，才能保障映射的**正确性**和**最小性**。
...
我们要把 `unknown` 换成一个**更精巧的类型**，这样用户在使用 `curry` 返回的结果（转换后的函数）时，就能够获得正确的类型提示了。

然而，更精巧的类型，具体是什么，取决于调用 `curry` 时传入的那个函数。我们使用**泛型约束**把传入的那个函数的参数 `P` 和返回类型 `R` 提取出来：

```typescript
type Curry = <P extends unknown[], R>(toCurry: (...args: P) => R) => Curried<P, R>;
```

然后，将 `P` 和 `R` 传递 `Curried` 泛型，作为转换函数的返回类型（也就是前述的那个更精巧的类型）。
...
TS 没有报错或陷入死循环，而是仍然映射出了一种可以无穷调用下去的函数类型。所以，递归时逐渐缩减规模并不是泛型递归的必要条件，泛型的某种**惰性**允许我们去为诸如 JS 中的**循环引用对象**或**返回自身的函数**定义类型：
...
虽然 `CurriedV1` 是最简单的柯里化，但已经足够装下本文中大部分关于「从集合视角看泛型」的领悟。接下来，我会加快一些速度，把更高级的柯里化的类型实现讲完。

### CurriedV2：允许不定项参数

如果柯里化转换后的函数，可以接收不定项个参数（形如 `curried(1)(2,3)(4)`），就会更易用一些。我的实现是：
...
注意，`Prepend` 不是条件类型，自然不满足分散条件类型，所以 `Prepend_Test3` 是 `[1 | 2, ...3[]]` 而不是 `[1, ...3[]] | [2, ...3[]]`。如果你想要得到后者，可以将 `Prepend` 的实现放在条件类型内，如下所示：

```typescript
type DistributedPrepend<E extends unknown, T extends> = E extends unknown ? [E, ...T] : never;

type Prepend_Test4 = DistributedPrepend<1 | 2, 3[]>;    // ==> [1, ...3[]] | [2, ...3[]]
```

> 本文后续讨论假设所有传入的类型都是不分散的（即非并集的形式），也不再讨论分散条件类型的问题。
...
故事要从 TS 的官方泛型 `Partial` 开始说起： `Partial` 泛型可以将一个对象类型的所有属性都变得可选。当它作用于数组时，会使数组的每一项变成可选，比如 `Partial<[number, string]>` 可以得到**类似**于 `[number?, string?]` 的类型。

我们期望 `CurriedV2` 支持不定项参数，因此需要从定项参数元组类型中抽取出「元组的前任意项」类型：比如定项参数是类型 `[1, 2, 3]`，那么不定项参数可以是 `[1]`，`[1, 2]` 或者 `[1, 2, 3]`。然而，TS 目前的类型运算没办法实现「元组的前任意项」这样的映射规则，而 `Partial` 是最接近的实现（最小超集）。
...
原文直接使用 `Partial` 而不报错，这是 TS 的一个 bug：对于 `Partial` 传入元组类型后，究竟还是不是元组，在不同的条件下判断不一致。我已经提交了 [issue](https://github.com/microsoft/TypeScript/issues/47128) 和[最简复现](https://www.typescriptlang.org/play?ts=4.1.5#code/C4TwDgpgBAIgjAHgAoD4oF4pKhAHsCAOwBMBnKAV0IGtCB7Ad0IG0BdKAfigAoA6fgIYAuLAIBOwAJYCANshQBKDGgBudScSgjCEFRDEBuKAHpjOMWLpiAsACg7oSLABMyHPiJlKNekzZpMPkERJHEpWXkldFV1YiNTKFIKAGNkiFJSOzsEgBUAC2hgBjp3AQBbMBloUjy6ChlNACM6YDzElLSMiE0rKAAzAUkq4izbIA)，根据维护者的反馈，这应该是一个由已知深层原因导致的问题。
...
最重要的一点区别是，`CurriedV2` 为转换后函数引入了泛型约束，这样每次调用时，就能动态提取出传入参数的数量，并据此（参数数量）计算此次调用返回的类型。
...
简单测试，我们发现 `CurriedV2_Test1` 无法直白给出柯里化转换后的函数的类型，因为每一步调用后得到类型，只有调用的时候才能（根据参数）确定。

### CurriedV3：支持剩余参数

有些函数的参数分为两个部分：固定参数和剩余参数。`CurriedV3` 可以支持这种「包含剩余参数的」函数的柯里化。

剩余参数都是可选的，你必须在最后一次调用时一次性传入所有剩余参数；否则柯里化无法判断，你是想要接着传入剩余参数，还是结束调用并获取结果。
...
`CurriedV3` 与 `CurriedV2` 的递归结束条件不同：满足 `P extends [unknown, ...unknown[]]` 可以推断出 `P` 仍然包含固定项，继续递归；不满足此条件说明 `P` 中只有剩余参数了，结束递归。

得益于严密的 `Drop` 以及背后的 `Tail` —— 它们妥善处理了包含剩余项元组的情况 —— `CurriedV3` 的递归部分和 `CurriedV2` 是完全一致的。
...
泛型 `Equal` 判断两个类型是不是完全相等（仍然是集合运算，`true` 和 `false` 表示包含布尔值的单元素集合）。
...
先看 `Reverse` 实现的核心部分（从 `T extends` 开始）：接收数组类型 `T`，递归地调用自己，每次递归将 `T` 的头元素取下来，从头部推入 `R` 中。当 `T` 消耗殆尽，`R` 自然就是翻转后的数组。

对于固定长度的元组类型，这样做没问题。但是，如果想要翻转不固定长度的数组类型呢？

通过简单的归纳，我们知道 `Reverse<string[]>` 就是 `string[]`，映射仍然是完美的；对于 `Reverse<[string, ...number[]]>`，我们只能将其映射为 `Array<string | number>` —— 我们之前说过，泛型的返回时常比我们预期的类型要宽泛，这是不可避免的。

`Reverse` 实现的前两行，处理了上述两种不固定长度的数组类型，其中泛型 `Item` 帮助我们从数组类型中提取出数组项的可能类型。
...
泛型 `PlaceholderMatched` 将元组 `T` 中的类型为 `M` 的元素提取出来，并从元组 `S` 中相同位置提取出元素类型，存放在一个数组里，并最终返回。
...
简单看一下测试就知道 `PlaceholderMatched` 的具体作用了：
...
和 `CurriedV3` 的区别在递归部分，我们用 `PlaceholderTuple<P, __>` 约束函数的入参，这样调用者就可以传入占位符常量 `__` 了。在单次递归中，我们将「被占位的元素」构成的元组类型提取出来（即 `PlaceholderMatched<T, P, __>`），并且与此次消耗参数后剩余的参数（即 `Drop<Length<T>, P>>`）连接起来，作为新的参数 `P`。
...
虽然本文中，对集合的讨论主要集中在前半部分（`CurriedV1` 相关小节），但是促使我去思考的，其实是针对后面几个更高级的场景的实践。
...
原文的很多基础类型，都有没处理妥善的边缘用例，所以当问题越来越复杂之时，泛型实现就会越来越不可控。后来原作者开始引入 `Cast` 泛型，把推导到边缘的不准确的类型强行转换回来。
